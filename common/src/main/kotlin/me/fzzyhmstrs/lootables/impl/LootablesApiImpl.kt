...
internal object LootablesApiImpl {
    ...
    internal fun supplyLootWithChoices(tableId: Identifier, playerEntity: ServerPlayerEntity, origin: Vec3d, onSuccess: BiConsumer<ServerPlayerEntity, Vec3d> = BiConsumer { _, _ -> }, onAbort: BiConsumer<ServerPlayerEntity?, Vec3d> = BiConsumer { _, _ -> }, key: IdKey?, rolls: Int = 3, choices: Int = 1): Boolean {
        ...
        if (key != null && !LootablesData.keyAvailable(key, playerEntity)) {
            return false
        }
        ...
        val pendingPoolChoices = LootablesData.getPendingPools(choiceKey, playerEntity)
        val poolChoices = if(pendingPoolChoices != null) {
            pendingPoolChoices
        } else {
            val storedPoolChoices = LootablesData.getStoredPools(choiceKey, playerEntity)
        ...
        if (key != null)
            LootablesData.applyKey(key, playerEntity)
    ...
    internal fun canApplyKey(key: IdKey, playerEntity: ServerPlayerEntity): Boolean {
