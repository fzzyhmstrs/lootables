...
class ValidatedChoiceStyle: ValidatedEnum<ChoiceStyle>(ChoiceStyle.HORIZONTAL) {
    ...
    //client
    private class ChoiceStylesOptionsWidget(choicePredicate: ChoiceValidator<ChoiceStyle>, private val entry: Entry<ChoiceStyle, *>)
        :
        CustomPressableWidget(0, 0, 110, 20, entry.get().let { it.transLit(it.name) }), SpriteDecorated {
        ...
        override fun textures(): TextureProvider {
            return TextureSet.Single(entry.get().id)
        }
